...
class DetailMsgTableViewCell: UITableViewCell {
    var doctor: Doctor? {
        didSet {
            if let doctor = doctor {
                details = [doctor.name, "\(doctor.likeNum)", doctor.hospital, "\(doctor.workAge)年工作经验", doctor.level, doctor.introduction]
                for i in 0..<detailLabels.count {
    ...
    private var details = ["上海地区医生", "6004268", "上海第一人民医院", "7年工作经验", "副主任医师", "胃肠肝胆胰手术前后、肿瘤、放化疗、胰腺炎、脂肪泻、消瘦等疾病的营养干预和治疗。多次获得奖励,发表专业相关文章多篇,尤其擅长根据个体疾病特点及生理阶段为不同病患提供个性化的营养咨询和饮食指导。初诊时间:每周五上午:9:00-12:00"]
